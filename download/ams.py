...
class AMSDownloader(BaseDownloader):
    ...
    def _get_pids_from_df(self, df, id_columns=None):
        ...
        if False:  # Temporarily disable cache for lat and lon
        ...
        if False:  # Temporarily disable cache for raw pids
            ...
            def worker(row):
                pids = self._get_raw_pids_with_ts(row.latitude, row.longitude, self.buffer)
                result = []
                for pid in pids:
                    result.append(
                        {
    ...
    def _get_raw_pids_with_ts(self, lat, lon, buffer):
        """Get raw panorama IDs from the Amsterdam Street View API."""
        url = f"https://api.data.amsterdam.nl/panorama/panoramas/?near={lon},{lat}&radius={buffer}&srid=4326"
        ...
        for attempt in range(10):
            try:
                ...
                data = json.loads(response.content)
                panoramas = data["_embedded"]["panoramas"]
                return [(item["pano_id"],item['timestamp']) for item in panoramas]
            except Exception as e:
                if attempt == 9:  # Last attempt
                    warnings.warn(f"Failed to get panorama IDs after 5 attempts: {e}")
                    return []
    ...
    def _filter_pids_date(self, pid_df, start_date, end_date):
        ...
        pid_df["date"] = pd.to_datetime(pid_df["timestamp"])
        return pid_df[(pid_df["date"] >= start_date) & (pid_df["date"] <= end_date)]

    def _closest_pids_date(self, pid_df, start_date, end_date):
        """Filter panorama IDs by closest date."""
        pid_df["date"] = pd.to_datetime(pid_df["timestamp"], utc=True, errors='coerce')
        ...
        # Group by lat_lon_id and find the closest image to center date for each group
        result_rows = []
        for lat_lon_id in pid_df['lat_lon_id'].unique():
            group = pid_df[pid_df['lat_lon_id'] == lat_lon_id]
            group = group.copy()

            group = group.dropna(subset=['date'])
            
            # Skip if no valid dates remain
            if group.empty:
                continue

            group['date_diff'] = (group['date'] - center_dt).abs()
            closest_idx = group['date_diff'].idxmin()
            result_rows.append(group.loc[closest_idx])
    ...
    def download_svi(
        ...
        if pid_df.shape[0] == 0:
            print("No images found. Please check the input parameters.")
            return

        if start_date and end_date:
            pid_df = self._closest_pids_date(pid_df, start_date, end_date)

        def process_pid(pid, image_id, max_retries=5):
                ...
                try:
                    ...
                    if response.status_code == 200:
                        if not metadata_only:
                            self._save_image(image_id, data, cropped)
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            futures = [executor.submit(process_pid, row.pano_id, row.image_id) for _, row in pid_df.iterrows()]
            for future in verbosity_tqdm(
